  ...
  if (improverForm) {
    improverForm.addEventListener('submit', async (e) => {
      ...
      if (!bulletText || bulletText.length < 5) {
        showToast('Please enter a sentence or project bullet to improve.', 'warning');
        return;
      }
      ...
      try {
        ...
        if (response.ok && data.success) {
          ...
          showToast(data.error || 'Failed to improve bullet point.', 'danger');
        }
      } catch (err) {
        showToast('Network error while contacting AI service.', 'danger');
      } finally {
        if (improveBtn) {
    ...
    copyImprovedBtn.addEventListener('click', () => {
      ...
      if (text) {
        navigator.clipboard.writeText(text).then(() => {
          showToast('Enhanced bullet copied to clipboard!', 'success');
        });
      }
    });
  }
